"""
文件名: test_coupon_flow.py
作用:测试优惠券下单流程
"""
import allure
import pytest

from Interface.CouponInterface import CouponInterface
...
@allure.feature('优惠券')  # 模块名称
@allure.story('优惠券')  # 测试模块下的功能
@allure.title('优惠券的添加和使用')  # 用例标题
...
def test_coupon_flow():
    ...
    delete_coupon_sql = """DELETE FROM sms_coupon_history WHERE member_id=146"""  # 查询用户id为id=146的用户领取的优惠券
    stock_sql = """SELECT stock FROM pms_sku_stock WHERE id = 98"""  # 查询id为98的商品库存
    ...
    Database().write(delete_coupon_sql)  # 删除用户已领取的优惠券
    # 3.1 领取优惠券
    select_sql = """select * from sms_coupon"""  # 查询可领取的优惠券列表
    select_coupon = Database().readone(select_sql)  # 执行查询优惠券sql
    coupon_result = CouponInterface().add_coupon(29)  # 执行领取指定优惠券接口
    coupon_code = GetKeyword.get_keyword(coupon_result, 'code')  # 获取领取指定优惠券接口返回开发自检字段
    pytest.assume(coupon_code == 200)  # 断言开发自检字段 code

    # 3.2 获取优惠券列表查看未使用的优惠券
    use_coupon = CouponInterface().coupon_list(0)
    coupon_id = GetKeyword.get_keyword(use_coupon, 'couponId')  # 获取优惠券列表接口返回开发自检字段
    pytest.assume(coupon_id == 29)  # 断言开发自检字段 couponId
    ...
    # 3.3 查看会员购物车的相关优惠券
    cart_coupon = CouponInterface().cart_coupon_list('1')  # 执行获取会员购物车的相关优惠券接口
    coupon_cart = GetKeyword.get_keyword(cart_coupon, 'code')  # 获取购物车相关优惠券列表接口返回开发自检字段
    pytest.assume(coupon_cart == 200)  # 断言开发自检字段 code
    ...
    confirm_order_result = order.confirm_order()
    amount = GetKeyword.get_keyword(confirm_order_result, 'amount')  # 优惠金额
    pytest.assume(amount == 30)  # 断言优惠金额=优惠券优惠金额
