...
class TestOrders:
    ...
    @pytest.mark.parametrize(
        "channel, locale, cart_checkout,promotion_code,membership_name, fulfillment, receive_info, voucher_ids, user_remarks,sellables,Expected_Results",
        yml_make_payment_data, ids=ids_make)
    def test_create_make_payment_order(self, get_token, channel, locale, cart_checkout,promotion_code,membership_name, fulfillment, receive_info, voucher_ids,
                          user_remarks, sellables,Expected_Results):
        with allure.step('Get the return value of the interface'):
            r_detail = self.Orders.create_make_payment_order(get_token, channel, locale,cart_checkout,promotion_code,membership_name, fulfillment, receive_info, voucher_ids, user_remarks,sellables)
        with allure.step('assertion'):
